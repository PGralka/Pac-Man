...
void Player::move() {
	...
	horizontalLoop();
	centre = QPointF((2 * pos().x() + boundingRect().width()) / 2,
					 (2 * pos().y() + boundingRect().height()) / 2);
}

void Player::keyPressEvent(QKeyEvent* e) { pressedKey = e->key(); }
